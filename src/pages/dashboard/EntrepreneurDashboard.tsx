...
export const EntrepreneurDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [collabRes, investorsRes] = await Promise.all([
          api.get('/collaborations'),
          api.get('/users/investors'),
        ]);
        setCollaborationRequests(collabRes.data);
          ...
          <CardBody>
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-xl font-semibold text-success-900">0</h3>
              </div>
            </div>
          </CardBody>
        </Card>
          ...
          <Card>
            ...
            <CardBody className="space-y-4">
              {recommendedInvestors.map(investor => (
                <InvestorCard key={investor.id || (investor as any)._id} investor={investor} showActions={false} />
              ))}
            </CardBody>
          </Card>
        </div>
